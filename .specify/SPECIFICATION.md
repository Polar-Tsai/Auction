...
若新價格小於 原起標價 / 2 → 立即撤標（標記為 Unsold 並通知 Admin），由 Admin 決定後續處理
```

**流標認定條件**:
```
若在最低起標價 (10 元) 或經過撤標條件觸發後，仍無人出價超過 1 小時（或符合撤標條件已執行），系統將標記為 "Unsold"，並通知 Admin 以便後續處理。
```
...
for each 商品 in 進行中的商品:
  ...
  if 新起標價 < (原起標價 / 2):
    標記該商品為 Unsold
    記錄原因為 "Price fell below half of start price - auto withdrawn"
    通知 Admin 檢視並決定後續（可重新上架、手動調價或永久下架）
    拍賣狀態改為 "Ended"
    continue
