...
#### 2.1.2 User（公司同仁）
**身份驗證** (更新於 2026-01-22)
- 輸入 **Email 前綴**（帳號）+ **工號**（密碼）
- 系統自動補完 `@kingsteel.com` 並從 employees.csv 匹配對應的 Email
- 驗證成功 → 檢查輸入的「密碼」是否等於該員工的「工號」(employeeId)
...
- 在商品詳情頁出價區域顯示當前登入資訊：  
  「當前登入：工號 [XXX] - [姓名]」
- 密碼為員工工號，例如：1305 代表工號為 1305 的員工
...
- 如果未登入
  - Email 前綴輸入框（account）
  - 右側顯示固定的 `@kingsteel.com`
  - 工號輸入框（type="password"，作為密碼使用）
  - 登入按鈕
  - 錯誤提示（帳號或工號錯誤/網路錯誤）
...
Email 前綴 + 工號輸入 → 點擊登入
    ↓
後端處理
    ├─ 移除前後空格，補齊完整 Email (@kingsteel.com)
    └─ 比對 employees.csv
    ↓
    ├─→ Email 存在 AND 工號 (employeeId) 正確 
...
- **觸發時機**：用戶在系統中第一次進行出價時
- **UI 元素**：
  - 藍色圓形 i 圖標（品牌色 #1E40AF）
  - 標題：「首次出價確認」
  - 說明文字：清楚告知這是首次競標，確認後將記錄出價
  - 兩個按鈕：「取消」（灰色邊框）/ 「我確認參加」（藍色實心）
- **z-index**：200（確保在所有元素之上，包括手機版置底出價區域）
- **多語言支援**：繁體中文 + 印尼文
- **資料寫入時機**：
  - ✅ 只有點擊「我確認參加」後才寫入出價記錄
  - ❌ 點擊「取消」或關閉對話框不會寫入任何資料
- **後續行為**：用戶確認後，該用戶不會再看到此對話框

**實時更新** (更新於 2026-01-09):
- 頁面每 2 秒自動 Polling 一次
- 獲取最新出價、結標倒數計時、商品狀態、**最高出價者資訊** (v1.2 修復)
- 若有新出價 → 刷新頁面資料
- 若時間到 → 自動跳轉到結果頁
- **商品列表頁即時更新** (v1.2)：
  - 進行中商品的最高出價者工號會即時更新
...
| 版本 | 日期 | 變更說明 |
|------|------|---------|
| 1.2 | 2026-01-09 | 新增首次出價確認功能；修復商品列表頁最高出價者即時更新問題 |
| 1.1 | 2026-01-05 | 新增密碼登入功能；固定增量出價模式 |
